...
nombres = c('Mauricio','Fernando', 'Juan', 'Pedro','Marco', 'Clara')
nombres

#Definir un factor, el factor tiene niveles, cada elemento del factor es igual a uno de los niveles
#Se puede decir que es una lista que tiene copias de las etiquetas colocadas
#Todo lo quen queramos convertir de texto a una forma de clasificación usamos los factores
#Es parecido a un diccionario de python pero solo con las llaves
nombres.factor = factor(nombres)
nombres.factor

gender = c("M","H","M","M","M","H")
...
#Específicamos un nivel más, el nivek B
...
#Cambiar nombre directamente a los niveles del factor
levels(gender.fact4) = c("Femenino","Masculino","Híbrido")
gender.fact4

notas <- c(3,4,1,2,5,3,4,2)
notas.factor = factor(notas)
levels(notas.factor)
notas.factor
levels(notas.factor) = c("Pésimo","Deficiente","Regular","Bueno", "Excelente")
notas.factor

#Aprobar distintos grupos cambiando los niveles
levels(notas.factor) = c("Deficiente","Deficiente","Regular","Bueno","Bueno")
notas.factor

#Factor ordenado
notas
ordered(notas, labels = c("Def","Def","Reg","Bue","Bue"))
#Orden de los levels Def < Reg < Bue
